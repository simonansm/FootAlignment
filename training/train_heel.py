"""Training script for foot heel classification."""
...
def setup_paths():
    ...
    return checkpoint_dir, last5_dir
...
def main():
    """Main training loop."""
    root = get_project_root()
    checkpoint_dir, last5_dir = setup_paths()
    
    # Paths
    csv_file = root / "data" / "foot_3class.csv"
    image_dir = root / "archive" / "Plantar_dataset"
    best_model_path = checkpoint_dir / "best_model_inception_heel.pt"
    ...
    train_loader, val_loader, test_loader, full_df, train_idx = load_data(
        csv_file, image_dir, "GT_label_heel", transform
    )
    
    # Class weights
    train_labels = full_df.loc[train_idx, "GT_label_heel"].values
